...
class MatrixModule(BotModule):
    ...
    async def matrix_message(self, bot, room, event):
        ...
        if not args:
            return await bot.send_text(room, event, 'Missing dice argument')
        res = []
        bonus = 0
        try:
            for arg in args:
            ...
            return await bot.send_text(room, event, f'Invalid dice spec: {arg}. Use the form [count]d[sides]')

        # only one die
        if len(res) < 2 and not bonus:
            return await bot.send_text(room, event, str(res[0]))
        ...
        if bonus > 0:
            ...
            lhs += f'{bonus}'
        return await bot.send_text(room, event, f'{lhs} = {rhs}')
